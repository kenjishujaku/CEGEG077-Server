...
app.get('/getGeoJSONfile',function(req,res){
	...
	pool.connect(function(err,client,done){
		...
		querystring = querystring + " row_to_json((SELECT l FROM (SELECT first_name, last_name, module_code,question,choice_1,choice_2,choice_3,choice_4,correct_answer,location_name) As l )) As properties";
...
app.post('/updateData',function(req,res){
  ...
  pool.connect(function(err,client,done){
	  ...
	  var querystring = "INSERT into app_userdata(first_name,last_name,module_code,location_name,location) values ('";
	querystring = querystring + req.body.firstname + "','" + req.body.lastname + "','" + req.body.module + "','";
	querystring = querystring +req.body.pointname+"',"+geometrystring + "));";
	console.log(querystring);
	...
	client.query( querystring,function(err,result){
		...
		res.status(200).send("Update Has Completed");
	});
  }); 
});
